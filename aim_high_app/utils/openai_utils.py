...
import os
from . import Hidden_Prompt_Info

load_dotenv()
client = Client(host=os.getenv("OLLAMA_HOST_URL"))

def get_chatbot_response(messages, summary=None, response_format=Hidden_Prompt_Info.BaseJson):
    """
...
def generate_summary(content, title, response_format=Hidden_Prompt_Info.BaseJson): # Should be replaced with Vanorah's code
    """
...
def extract_concepts(content, title, response_format=Hidden_Prompt_Info.BaseJson): # Should be replaced with Briana's code
    """
...
def evaluate_summary(original_content, summary_content, response_format=Hidden_Prompt_Info.EvaluateSummaryFeedback):
    """
    ...
    try:
        response = client.chat(
            ...
            messages=[
                ...
                {"role": "user", "content": f"Original content: {original_content}\n\nSummary: {summary_content}\n\nPlease evaluate the summary on a scale of 1-5 for content quality, accuracy, and completeness. Also identify any key concepts that are missing in the summary. Format your response as JSON with 'score', 'feedback', and 'missing_concepts' properties. In missing_concepts, make it a list of ONLY the names of the concepts, do not put any description."}
            ],
            format=response_format.model_json_schema()
        )
